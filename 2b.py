...
Original file is located at
    https://colab.research.google.com/drive/1v3xNMpjnqv6Y9FCYYosxmipLg0JkWzJl
"""
...
def p_regression(data_x, data_y, nb_features, nb_samples):
    

    tf.reset_default_graph()
    data_x = create_feature_matrix(data_x, nb_features)
    data_y = create_feature_matrix(data_y, nb_features)
    ...
    # koristimo adam optimazer jer je bolji za polinomijalnu
...
def main():
    ...
    for i in range(MAX_FEATURES):
        xs, hyp_val, final_loss = p_regression(data_x, data_y, nb_features, nb_samples)
        plt.plot(xs[:, 0].tolist(), hyp_val.tolist(), color=colors[i], label='deg(p)={}'.format(nb_features))
...
"""Krive prvog i drugog stepena lose fituju podatke, za razliku od polinoma veceg stepena. Najbolje je izabrati stepen polinoma 3 jer je funkcija troska znacajno manja"""